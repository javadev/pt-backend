...
public class AdminProgramAssignServiceTest {
    ...
    @Mock private InWorkoutItemSetRepository inWorkoutItemSetRepository;
    @Mock private ParseProgramRepository parseProgramRepository;
    @Mock private InWarmupWorkoutItemRepository inWarmupWorkoutItemRepository;
    @Mock private DictionaryService dictionaryService;
    @Mock private ExerciseRepository exerciseRepository;

    @InjectMocks private AdminProgramAssignService adminProgramAssignService;

    @Test
    public void assign_user_group_1_two_goals() {
        when(parseProgramRepository.findAll(any(Sort.class)))
                .thenReturn(Arrays.asList(getParseProgram()));
        when(dictionaryService.getEnValue(eq(DictionaryName.goal_title), isNull(), isNull()))
                .thenReturn("Loose weight");
        adminProgramAssignService.assign(
                new InUser()
                        .setD_level("2")
                        .setD_sex("male")
    ...
    public void getCurrentWorkoutIndex_empty_programs() {
        assertThat(
                adminProgramAssignService.getCurrentWorkoutIndex(
                        new InUser().setInPrograms(Collections.emptyList()),
    ...
    public void getCurrentWorkoutIndex_empty_programs2() {
        assertThat(
                adminProgramAssignService.getCurrentWorkoutIndex(
                        new InUser().setInPrograms(null), Collections.emptyList()),
                equalTo(0));
    }
                                                                ...
                                                                Arrays.asList(
                                                                        ...
                                                                        new InWorkout()
                                                                                .setD_workout_name(
                                                                                        "name2"))))),
                        Arrays.asList(
                                new ParseWorkout().setName("name1"),
                                new ParseWorkout().setName("name2"))),
                equalTo(1));
    }
    ...
    public void multiplyLists_two_elements() {
        List<ParseRound> parseRounds1 =
                Arrays.asList(new ParseRound().setName("1_1"), new ParseRound().setName("1_2"));
        List<ParseRound> result = adminProgramAssignService.multiplyLists(parseRounds1, 1);
        assertThat(result.size(), equalTo(2));
        assertThat(result.get(0).getName(), equalTo("1_1"));
    ...
    public void mergeLists_one_element() {
        ...
        List<ParseWorkout> result =
                adminProgramAssignService.mergeLists(parseWorkouts1, Collections.emptyList());
        assertThat(result.size(), equalTo(1));
        assertThat(result.get(0).getName(), equalTo("1_1"));
    }
    ...
    public void mergeLists_one_element2() {
        ...
        List<ParseWorkout> result =
                adminProgramAssignService.mergeLists(Collections.emptyList(), parseWorkouts2);
        assertThat(result.size(), equalTo(1));
    ...
    public void mergeLists_two_elements() {
        ...
        List<ParseWorkout> parseWorkouts2 = Arrays.asList(new ParseWorkout().setName("2_1"));
        List<ParseWorkout> result =
                adminProgramAssignService.mergeLists(parseWorkouts1, parseWorkouts2);
        assertThat(result.size(), equalTo(2));
        assertThat(result.get(0).getName(), equalTo("1_1"));
    ...
    public void mergeLists_five_elements() {
        List<ParseWorkout> parseWorkouts1 =
                Arrays.asList(new ParseWorkout().setName("1_1"), new ParseWorkout().setName("1_2"));
        List<ParseWorkout> parseWorkouts2 =
                Arrays.asList(
                        new ParseWorkout().setName("2_1"),
    ...
    public void mergeLists_five_elements2() {
        List<ParseWorkout> parseWorkouts1 =
                Arrays.asList(
                        new ParseWorkout().setName("1_1"),
                        new ParseWorkout().setName("1_2"),
                        new ParseWorkout().setName("1_3"));
        List<ParseWorkout> parseWorkouts2 =
                Arrays.asList(new ParseWorkout().setName("2_1"), new ParseWorkout().setName("2_2"));
        List<ParseWorkout> result =
                adminProgramAssignService.mergeLists(parseWorkouts1, parseWorkouts2);
        assertThat(result.size(), equalTo(6));
        assertThat(result.get(0).getName(), equalTo("1_1"));
                ...
                .thenReturn(
                        Arrays.asList(
                                new Exercise()
                                        .setExerciseTypes(
                                                Arrays.asList(
                                                        new ExerciseType().setName("test")))));
        assertThat(
                adminProgramAssignService.getExerciseType(new ParseWorkoutItem()), equalTo("test"));
    }
                ...
                .thenReturn(
                        Arrays.asList(
                                new Exercise()
                                        .setExerciseTypes(
                                                Arrays.asList(
                                                        new ExerciseType().setName("test1"),
                                                        new ExerciseType().setName("test2")))));
        assertThat(
                adminProgramAssignService.getExerciseType(
                        new ParseWorkoutItem()
                                .setParseWorkoutItemSets(
                                        Arrays.asList(
                                                new ParseWorkoutItemSet().setTime_in_min(1F)))),
                equalTo("OnTime"));
    }
        ...
        assertThat(
                adminProgramAssignService.getExerciseType(
                        new ParseWorkoutItem()
                                .setParseWorkoutItemSets(
                                        Arrays.asList(
                                                new ParseWorkoutItemSet().setTime_in_min(null)))),
