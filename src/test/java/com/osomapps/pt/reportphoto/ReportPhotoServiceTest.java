...
public class ReportPhotoServiceTest {
    ...
    @InjectMocks private ReportPhotoService reportPhotoService;
    ...
    public void create() {
        InUserLogin inUserLogin = new InUserLogin();
        InUser inUserForLogin =
                new InUser().setInUserGoals(Arrays.asList(new InUserGoal().setGoalId(1L)));
        inUserForLogin.setId(10L);
        inUserLogin.setInUser(inUserForLogin);
        when(userService.checkUserToken(eq("1"))).thenReturn(inUserLogin);
        when(inUserPhotoRepository.save(any(InUserPhoto.class)))
                .thenReturn(new InUserPhoto().setId(1L));
